...
public class GRAS {
	...
	private static String lexiconFilePath = "lexicons.txt"; //this is the address to the lexicon (a.k.a 'terms vector');
	...
	private static void computeFrequentSuffixPairs() throws FileNotFoundException{
		...
		while (scan.hasNextLine()){
			...
			if(term.length() >= lambda){
				if (array.size() == 0){
					prefix = term.substring(0,lambda);
					array.add(term);
				}else{
					...
					String currentPrefix = term.substring(0,lambda);
	...
	private static String longestCommonSubstring(String left, String right){
		String common = left.substring(0,lambda);
		for(int i = lambda; i < left.length() && i < right.length(); i++){
